...
class FormsController extends Controller {
	...
	public function actionIndex() {
		...
		\Yii::$app->view->title = 'ACA Reporting Service | Manage ACA Reporting Forms';
			...
			if (! empty ( $get_company_id['c_id'] )) {
				...
				&& empty ( $arr_validations ['benefit_plan'] ) 
				&& empty ( $arr_validations ['plan_class_validation'] ) 
				&& empty ( $arr_validations ['payroll_data_validation'] ) 
				&& empty ( $arr_validations ['medical_data_validation'] )
				&& !empty ( $arr_validations ['plan_class'] )
				&& !empty ( $arr_validations ['payroll_data'] )
				&& !empty ( $arr_validations ['medical_data'] )
	...
	private function mergepdf($arr_files,$file_name){
		$pdf = new Pdf ( $arr_files,['command' => '/usr/local/bin/pdftk','useExec' => true,] );
		//$pdf = new Pdf ( $arr_files );
		$pdf->compress()->flatten()->saveAs($file_name );
		unset($pdf);
	
		foreach ($arr_files as $each_file){
							...
							/************************************** part 2****************************************/	
								...
								if(!empty($post_values ['part3'])){
									...
									if($post_values ['part3']['section_4980h_fulltime_employee_count_all_III']!=''){
										$post_values ['part3']['section_4980h_fulltime_employee_count_all_III'] = preg_replace('/[^0-9]/', "", $valuefulltimeall);
									...
									if($countmonth >= 12){
										$post_values ['part3']['section_4980h_fulltime_employee_count_all_III'] = preg_replace('/[^0-9]/', "", $valuejan);
										foreach($arrmonth as $key=>$value){
										$post_values ['part3']['section_4980h_fulltime_employee_count_'.$value.'_III'] = '';
										}
									}else{
										...
										foreach($arrmonth as $key=>$value){
											$month = $post_values ['part3']['section_4980h_fulltime_employee_count_'.$value.'_III'];
											
											$post_values ['part3']['section_4980h_fulltime_employee_count_'.$value.'_III'] = preg_replace('/[^0-9]/', "", $month);
								...
								$part_2 = serialize ( json_encode ( $post_values ['part2'] ) );
									...
									if(!empty($post_values ['part3']) && (isset($post_values ['part3']['employer_self_insured__III']) && $post_values ['part3']['employer_self_insured__III']=='1')){
										...
										for ($i=17;$i<=34;$i++){
											...
											if(!empty($post_values ['xml']['section_2__'.$i][0]['first_name'])){
												$post_values ['part3']['name_'.$i.'_name_of_covered_individual__III']= $post_values ['xml']['section_2__'.$i][0]['first_name'] .' '.$post_values ['xml']['section_2__'.$i][0]['middle_name'] .' '.$post_values ['xml']['section_2__'.$i][0]['last_name'] .' '.$post_values ['xml']['section_2__'.$i][0]['suffix'];
											}else{
...
*/
	...
	*/
			...
			try{
				...
				//	$model_printmail->estimated_date=$values['estimated_date'];
					...
					if($values['person_type'] == 1){
						...
						$total = ($model_1095c_ssn * $arrtotvalues['price_value']);
					...
					$print_output = '';
						...
						if (! empty ( $model_pdfgenerate ) && empty ( $model_pdfforms )) {
							...
							$filepath = getcwd () . '/files/pdf/' . $encrypt_company_id . '/'.$encrypt_form_id.'/consolidated_1095' ;
							$file_name = $clean_company_name.'_'.time().'.zip';
						...
						$model_acaforms->update();
									
						$transaction->commit();
						$output['success']= $print_output;
						
						}else{
						//	print_r($model_printmail->errors()); die();
						}
		  ...
		  if(!empty($files_to_upload))
			{
				...
				foreach($files_to_upload as $file)
				{
					if($person_type == 1)
					  {
						  $pdf_path = 'employer/NOM_'.$file;
					  }
					  elseif($person_type == 2)
					  {
						   $pdf_path = 'employee/'.$file;
					  }
			...
			if(empty($result['error']))
			{
				 ...
				 \Yii::$app->CustomMail->Printandmail ( $print_details['to'], $print_details['from'], $print_details);
							...
							$result['error'] =  "Error in SFTP login.";
					}
	...
	final function actionCreateforms() {
		...
		if (! empty ( $all_companies )) 		// If companies
		{
			foreach ( $all_companies as $company ) {
				...
				//get company detail
				$check_company_details = $model_companies->Companyuniquedetails ( $company ['company_id'] ); // Company detail
				...
				try {
					if ($newform->validate () && $newform->save ()) {
						...
						if (! empty ( $resp1094 ['success'] ) && ! empty ( $resp1095 ['success'] )) {
							...
							/*end a mail to the user that the form generation is completed,
							send mail to account manager,
							and if no account manager then send to data@skyinsurance */
							...
							//prepearing array for form details
							$form_details["company_name"] = $check_company_details->company_name;
							$form_details["company_number"] = $check_company_details->company_client_number;
							$form_details["form_version"] = $company ['version'];
							...
							$recepients[0]['email'] =  'data@skyinsurancetech.com';
							$recepients[0]['name'] =  'data@skyinsurancetech.com';
							
							}
							
							$created_by_user_detail = TblAcaUsers::find()->select('useremail')->where(['user_id'=>$company ["created_by"]])->One();
							$created_by_name_details = TblAcaCompanyUsers::find()->select('first_name, last_name')->where(['user_id'=>$company ["created_by"],'client_id'=>$check_company_details->client_id])->One();
							$recepients[1]['name'] =  '';
							$recepients[1]['email'] = $created_by_user_detail->useremail;
							
							if(!empty($created_by_name_details)){
							$recepients[1]['name'] = $created_by_name_details->first_name.' '.$created_by_name_details->last_name;
							}
							
							
							\Yii::$app->CustomMail->Createforms($recepients,$brand_emailid,$form_details,$brand_phonenumber,$link_brandimage); 
	...
	private function actionCreate1094($company_id, $form_id, $created_by) {
		...
		try {
			...
			$sql="SELECT `tc`.`company_name`, `tbi`.`contact_first_name`, `tbi`.`contact_middle_name`, `tbi`.`contact_last_name`, 
					`tbi`.`contact_person_suffix`, `tbi`.`contact_person_title`, `tbi`.`contact_phone_number`, `tc`.`company_ein`, 
					`tbi`.`street_address_1`, `tbi`.`street_address_2`, `tbi`.`contact_state`, `tbi`.`contact_city`, `tbi`.`contact_country`, 
					`tbi`.`contact_zip` 
					FROM `tbl_aca_basic_information` `tbi` INNER JOIN `tbl_aca_companies` `tc` ON tc.company_id = tbi.company_id 
					WHERE tc.company_id='".$company_id."'";
			$employe_details = $connection->createCommand($sql)->queryOne();
		...
		*/
			...
			$sql="SELECT `assign_dge`, `dge_ein`, `street_address_1`, `street_address_2`, `dge_city`, `dge_name`, `dge_state`, `dge_zip`,
					 `dge_contact_first_name`, `dge_contact_middle_name`, `dge_contact_last_name`, `dge_contact_suffix`, 
					`dge_contact_phone_number` FROM `tbl_aca_designated_govt_entity` WHERE assign_dge=1 AND company_id='".$company_id."'";
			$dge_details = $connection->createCommand($sql)->queryOne();
		...
		*/
		...
		//print_r($result_unique_employees); die();
			...
			unset($result_unique_employees);
			
		/*
		 * fill data for field 19 to 22
		*/
			
			$sql=" SELECT `tag`.`is_authoritative_transmittal`, `tag`.`is_ale_member`, `tag`.`total_1095_forms`, `tpc`.`plan_offering_criteria_type`, 
					`tpi`.`renewal_month`, `tpc`.`company_certification_workforce`, `tpc`.`company_certification_medical_eligibility`, 
					`tpc`.`company_certification_employer_contribution`, `tag`.`total_aggregated_grp_months` FROM `tbl_aca_aggregated_group` `tag` 
					INNER JOIN `tbl_aca_plan_criteria` `tpc` ON tpc.company_id = tag.company_id 
					INNER JOIN `tbl_aca_general_plan_info` `tpi` ON tpc.company_id = tpi.company_id 
					WHERE tag.is_authoritative_transmittal=1 AND tag.company_id='".$company_id."'";
			$plan_details = $connection->createCommand($sql)->queryOne();
		 ...
		 */	
			...
			$sql=" SELECT mec_months FROM tbl_aca_mec_coverage WHERE company_id='".$company_id."'";
			$arr_mec_details = $connection->createCommand($sql)->queryOne();
			...
			$arr_individual_months_count = self::actionCountdetails ( $company_id, 0 );
				...
				for($k=0;$k<=11;$k++)
				{
					...
					unset($arr_individual_months_count); 
			...
			if (! empty ( $plan_details )) {
				 ...
				 */
				if ($plan_details ['total_aggregated_grp_months'] == '0,' || $plan_details ['total_aggregated_grp_months'] == '0') {
		...
		*/
			...
			if (! empty ( $part2 ) && $part2 ['is_ale_member_yes_II'] == 1) {
				...
				$sql=" SELECT `tagl`.`group_name`, `tagl`.`group_ein` FROM `tbl_aca_aggregated_group` `tag` 
						INNER JOIN `tbl_aca_aggregated_group_list` `tagl` ON tagl.aggregated_grp_id = tag.aggregated_grp_id 
						WHERE tag.is_authoritative_transmittal=1 AND tag.company_id='".$company_id."'";
				$arr_agg_list_details = $connection->createCommand($sql)->queryAll();
	...
	**/
	private function actionCountdetails($company_id, $months) {
		...
		$sql="SELECT employee_id,ssn FROM `tbl_aca_payroll_data` WHERE company_id='".$company_id."' GROUP BY ssn";
		$arr_get_all_unique_payroll_employee = $connection->createCommand($sql)->queryAll();
		...
		if (! empty ( $arr_get_all_unique_payroll_employee )) {
			...
			foreach ( $arr_get_all_unique_payroll_employee as $employee ) {
				...
				$sql="SELECT hire_date,termination_date,plan_class,status FROM `tbl_aca_payroll_employment_period` WHERE employee_id='".$employee['employee_id']."' ";
				$employment_periods = $connection->createCommand($sql)->queryAll();
				...
				if (! empty ( $employment_periods )) {
					...
					//	$medical_plan_class = $periods->planClass;
						...
						$sql="SELECT employee_medical_plan FROM `tbl_aca_plan_coverage_type` WHERE plan_class_id='".$periods['plan_class']."' ";
						$medical_plan_class = $connection->createCommand($sql)->queryOne();
		...
		unset($employment_periods);
		unset($arr_get_all_unique_payroll_employee);
		unset($medical_plan_class);
		...
		try {
			...
			if (! empty ( $arr_get_all_unique_payroll_employee )) {
				...
				foreach ( $arr_get_all_unique_payroll_employee as $employee ) {
					...
					$sql="SELECT hire_date,termination_date,plan_class,status,waiting_period FROM `tbl_aca_payroll_employment_period` WHERE employee_id='".$employee['employee_id']."' ";
					$employment_periods = $connection->createCommand($sql)->queryAll();
					...
					if (! empty ( $employment_periods )) {
						...
						foreach ( $employment_periods as $periods ) {
							...
							if ($periods['status'] == 1) {
								...
								$sql="SELECT employee_medical_plan,plan_type_doh FROM `tbl_aca_plan_coverage_type` WHERE plan_class_id='".$periods['plan_class']."' ";
								$medical_plan_class = $connection->createCommand($sql)->queryOne();
...
private function actionGeteligibiltydate($waiting_period, $start_date, $value_3_1_1=0, $status=1) {
		...
		switch ($waiting_period) {
			...
			case 5 :
				...
				//$days=1;
				$temp_eligible_date = date ( 'Y-m-'. $days, strtotime ($temptermdate) );
				$eligible_date = date ( 'Y-m-d', strtotime ( $temp_eligible_date . "+1 day") );
		...
		try {
			...
			if (! empty ( $result_unique_employees ['error'] )) {
				...
				if (! empty ( $result_unique_employees ['success'] [0] )) {
					...
					unset($get_all_payroll_employee);
				}
				
					/*
				 * block for medical data forms
				 */
				
				if(! empty ( $result_unique_employees ['success'] [1] )){

					
					/**
					 * ****assign variable*****
					 */
					$medical_resposible_ssn = $result_unique_employees ['success'] [1];
					
					if(! empty ( $result_unique_employees ['success'] [0] )){
						$payroll_responsible_array=$result_unique_employees ['success'] [0];
					}else {
						$payroll_responsible_array=array();
					}
					
					$arr_resposible_ssn=array_values(array_unique(array_diff($medical_resposible_ssn,$payroll_responsible_array)));
					...
					*/
					 ...
					 * ***Get employee details*******
					*/
					$get_all_medical_employee = TblAcaMedicalData::find ()->select ( 'employee_id, first_name, middle_name, last_name, suffix, ssn, address1, apt_suite, city, state, zip, dob' )->where ( [
							'company_id' => $company_id,
							'ssn' => $arr_resposible_ssn
							] )->groupBy ( 'ssn' )->all ();
						
					if (! empty ( $get_all_medical_employee )) 					// start of # section (i)
					{
						foreach ( $get_all_medical_employee as $employee ) {
							$get_employment_periods = '';
							$get_medical_data = '';
							$get_medical_enrollment_periods = '';
							...
							if (! empty ( $get_self_insured_plan )) {
								...
								if (! empty ( $get_connected_medical_data )) {
									...
									foreach ( $get_connected_medical_data as $connected_medical ) {
										...
										unset($get_all_medical_employee);
		...
		if (empty ( $check_cron )) {
			...
			if (! empty ( $forms )) {
				foreach ( $forms as $form ) {
					...
					if (! is_dir ( getcwd () . '/files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id.'/consolidated_1095')) {
						 mkdir (  getcwd () . '/files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id.'/consolidated_1095', 0777);
						}
					...
					if (! empty ( $arr_1094_pdf_data )) {
						...
						if (! empty ( $arr_1095_pdf_data )) {
							$total_1095=count($arr_1095_pdf_data);
							$present_1095_count=0;
							$merge_25_1095_files=array();
							$present_1095_150_count=0;
							$merge_150_1095_files=array();
							$times='0';
							$records=150;
							foreach ( $arr_1095_pdf_data as $each_pdf_form ) {
								...
								$ar_address_data=array_merge($address_data,$arr_masked_ssn_data);
								
								$file_address_name = 'files/temp_pdf/' . $i . '_1095_merged_filled.pdf';
								$masked_file_address_name = 'files/temp_pdf/' . $i . '_1095_masked_filled.pdf';
								$file_name='files/temp_pdf/' . $i . '_1095_merge_25.pdf';
								$final_file_name='';
								...
								if (isset ( $part3 ['name_23_ssn__III'] ) && $part3 ['name_23_ssn__III'] != '') {
									...
									$pdf->fillForm ( $ar_address_data )->compress()->flatten()->saveAs ( $masked_file_address_name );
								...
								$present_1095_count++;
								array_push($merge_25_1095_files,$masked_file_address_name);
								if($present_1095_count==25){
									$this->mergepdf($merge_25_1095_files, $file_name);
									array_push($merge_150_1095_files,$file_name);
									$merge_25_1095_files=array();
									//unset($merge_25_1095_files);
									$present_1095_count=0;
										
									$present_1095_150_count++;
										
									if($present_1095_150_count==6){
										$final_file_name = 'files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id . '/consolidated_1095/'.$clean_company_name.'_'.time().'_'.($records*$times+1).'_to_'. ($records*($times+1)).'.pdf';
								
										$this->mergepdf($merge_150_1095_files, $final_file_name);
										//unset($merge_150_1095_files);
										$merge_150_1095_files=array();
										$present_1095_150_count=0;
										$times++;
							...
							if(!empty($merge_25_1095_files) || !empty($merge_150_1095_files)){
								$final_forms=array_merge($merge_25_1095_files,$merge_150_1095_files);
								
								
								$final_file_name = 'files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id . '/consolidated_1095/'.$clean_company_name.'_'.time().'_'.($records*$times+1).'_to_'. ($total_1095).'.pdf';
									
								$this->mergepdf($final_forms, $final_file_name);
							}
					...
					if (file_exists ( 'files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id )) {
						...
						$result_zip = \Yii::$app->Sharefile->foldertofolderzip ( getcwd () . '/files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id . '/', getcwd () . '/files/pdf/' . $encrypt_company_id . '/' . $clean_company_name.'_'.$clean_version .'_'.$zip_time. '.zip' );
					
						if(!empty($result_zip) && $result_zip == 'success')
						{
							//zip 1095 files
							//$result_zip_1095 = \Yii::$app->Sharefile->zipaDirectory ( getcwd () . '/files/pdf/' . $encrypt_company_id . '/' . $encrypt_form_id . '/consolidated_1095/', getcwd () . '/files/pdf/' . $encrypt_company_id. '/' . $encrypt_form_id  . '/' . $company_id . '_' . $form_id . '_consolidate_masked_1095.zip' );
