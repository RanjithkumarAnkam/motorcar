...
function redirect(){
	url = '<?php echo Yii::$app->getUrlManager()->getBaseUrl(); ?>/client/payroll?c_id=<?php echo $_GET['c_id']; ?>';
	location=url;
}			 
                            </script>
<style>
...
<div class ="col-md-4 pull-right">	<span class="fontsize18">This screen will exist only for  </span><span id="future_date">
</span></div>
		...
		<div class="col-xs-12 panel-0">
			<div class="box">
				...
				<div class="col-sm-offset-10 col-md-offset-10"><div style=" font-weight: 700;"><input class="comparision-check-all" style="vertical-align: top;" type="checkbox"> Select All </div></div>
					<form method="post" name="test" id="override_data_screen" action="<?php echo \Yii::$app->getUrlManager ()->getBaseUrl ().'/client/payroll/comparision?c_id='.$_GET['c_id']; ?>">
...
<tr>
							...
							?>
							    ...
							    <td><input name="TblAcaComparePayrollData[<?php echo $each_record->employee->employee_id; ?>]" id="checkbox_id_<?php echo $i;?>" value="<?php echo $each_record->employee->employee_id.'_'.$each_record->employee->ssn.'_'.$each_record->line_number;?>" class="<?php if($new_ssn==true){?> comparision-checkbox <?php }?> check_<?php echo $each_record->employee->ssn; ?>" onchange="checkedId(<?php echo $each_record->employee->ssn;?>,<?php echo $i;?>);" type="checkbox"></td>
							</tr>
...
$i++;
					 ...
					 <a href="<?php echo Yii::$app->getUrlManager()->getBaseUrl(); ?>/client/payroll?c_id=<?php echo $_GET['c_id']; ?>" class="btn btn-default ">cancel</a>
